                ...
                if (res.length > 0 && res[0] === '1') {
                    ...
                    if (sha1(checkSumKey) === res[3]) {
                        callback(null, {
                            ...
                            redirect_url: res[2],
                            status: res[0] == successCode ? 'SUCCESS' : 'FAILED',
                            message: '123PAY:' + JSON.stringify(res)
                        });
                    } else {
                        callback({
                            reference_id: referenceId,
                            redirect_url: '',
                            status: 'FAILED',
                            message: '123PAY: CreatePayment response has an incorrect checksum'
                        });
                    }
                } else {
                    callback({
